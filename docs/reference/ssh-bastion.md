...
The NAS VM (192.168.0.15) serves as an SSH bastion/jump host for emergency access when Tailscale is unavailable.
...
- **Network connectivity**: Direct access to all VMs on 192.168.0.0/24
...
ssh -i ~/.ssh/ansible_homelab -o ProxyJump=ansible@192.168.0.15 ansible@192.168.0.12
...
| `inventory-bastion.ini` | Emergency access | ProxyJump through NAS (192.168.0.15) |
...
- **NAS must be accessible**: Bastion requires you can reach 192.168.0.15
...
### Cannot reach bastion (192.168.0.15)
...
ssh -i ~/.ssh/ansible_homelab ansible@192.168.0.15 "tailscale status"
```
...
ssh -v -o ProxyJump=ansible@192.168.0.15 ansible@192.168.0.12

# Verify NAS can reach target
ssh -i ~/.ssh/ansible_homelab ansible@192.168.0.15 "ping -c 1 192.168.0.12"
```
